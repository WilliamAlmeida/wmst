...
- Always preserve correct Brazilian Portuguese accents and diacritics in user-facing UI text, labels, buttons, headings, validation messages, alerts, empty states, and documentation shown inside the product. Do not normalize visible text by removing accents. Use `ação`, not `acao`; `coração`, not `coracao`; `é`, not `e`.
